...
import cn.edu.xmu.javaee.productdemoaop.mapper.link.ProductAllMapper_link;
...
public class ProductDao {
    ...
    private ProductAllMapper_link productAllMapper_link;
    @Autowired
    ...
    public List<Product> findProductByName_link(String name) throws BusinessException{
        List<ProductAllPo> productPoList = productAllMapper_link.findProductsByName_link(name);

        if (productPoList.isEmpty()) {
            throw new BusinessException(ReturnNo.RESOURCE_ID_NOTEXIST, "没有找到匹配的产品");
        }
    ...
    public Product findProductByID_link(Long productId) throws BusinessException {
        Product product = null;
        List<ProductAllPo> productPoList = productAllMapper_link.findProductById_link(productId);
        if (productPoList.size() == 0){
            throw new BusinessException(ReturnNo.RESOURCE_ID_NOTEXIST, "产品id不存在");
        }
        product = CloneFactory.copy(new Product(), productPoList.get(0));
